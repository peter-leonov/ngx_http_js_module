#include <ngx_config.h>
#include <ngx_core.h>
#include <ngx_http.h>
#include <nginx.h>

#include <ngx_http_jsapi.h>

#include <ngx_http_js_module.h>
#include <nginx_js_glue.h>
#include <classes/File.h>

#include <nginx_js_macroses.h>

#define TRACE_METHOD() \
	ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0, COLOR_CYAN "File#%s" COLOR_CLEAR "(fd=%p)", __FUNCTION__ + 7, fd);
#define TRACE_STATIC_METHOD() \
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0, COLOR_CYAN "File.%s" COLOR_CLEAR "()", __FUNCTION__ + 7);

// According to http://nginx.org/pipermail/nginx-devel/2010-April/000200.html.
// Binary >> and << is used to avoid JS_ASSERT((v & jsval(1)) == jsval(0));
// at jsobj.h:255 in debug mode.
#define FD_TO_PTR(fd)   ((void *) ((uintptr_t) fd << 1))
#define PTR_TO_FD(p)  ((ngx_fd_t) ((uintptr_t) p >> 1))

// introduced in nginx 0.8.36
#ifndef NGX_FILE_NONBLOCK
#define NGX_FILE_NONBLOCK O_NONBLOCK
#endif

JSObject *ngx_http_js__nginx_file__prototype;
JSClass ngx_http_js__nginx_file__class;
static JSClass *private_class = &ngx_http_js__nginx_file__class;

static size_t        read_leaks;
static size_t        open_files;

JSObject *
ngx_http_js__nginx_file__wrap(JSContext *cx, ngx_fd_t fd)
{
	JSObject     *file;
	
	TRACE();
	
	file = JS_NewObject(cx, &ngx_http_js__nginx_file__class, ngx_http_js__nginx_file__prototype, NULL);
	if (!file)
	{
		ngx_log_error(NGX_LOG_CRIT, ngx_cycle->log, 0, "could not create a wrapper object for fd (%l)", (long) fd);
		return NULL;
	}
	
	JS_SetPrivate(cx, file, FD_TO_PTR(fd));
	
	return file;
}

static JSBool
method_open(JSContext *cx, uintN argc, jsval *vp)
{
	ngx_fd_t         fd;
	JSString        *jss_name;
	JSObject        *file;
	char            name[NGX_MAX_PATH];
	size_t          len;
	
	TRACE_STATIC_METHOD();
	
	E(argc == 1, "Nginx.File#rename takes 1 mandatory argument: name:String");
	
	
	// converting smth. to a string is a very common and rather simple operation,
	// so on failure it's very likely we have gone out of memory
	
	jss_name = JS_ValueToString(cx, JS_ARGV(cx, vp)[0]);
	if (jss_name == NULL)
	{
		JS_ReportOutOfMemory(cx);
		return JS_FALSE;
	}
	
	len = JS_EncodeStringToBuffer(jss_name, name, NGX_MAX_PATH - 1);
	if (len == (size_t) -1 || len > NGX_MAX_PATH - 1)
	{
		return JS_FALSE;
	}
	
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "ngx_open_file(\"%s\")", name);
	fd = ngx_open_file(name, NGX_FILE_RDWR, NGX_FILE_CREATE_OR_OPEN, NGX_FILE_DEFAULT_ACCESS);
	
	if (fd == NGX_INVALID_FILE)
	{
		JS_SET_RVAL(cx, vp, JSVAL_NULL);
		return JS_TRUE;
	}
	
	file = ngx_http_js__nginx_file__wrap(cx, fd);
	if (file == NULL)
	{
		JS_ReportOutOfMemory(cx);
		return JS_FALSE;
	}
	
	open_files++;
	
	JS_SET_RVAL(cx, vp, OBJECT_TO_JSVAL(file));
	return JS_TRUE;
}

static JSBool
method_rename(JSContext *cx, uintN argc, jsval *vp)
{
	JSString        *jss_from, *jss_to;
	char            from[NGX_MAX_PATH], to[NGX_MAX_PATH];
	size_t          len;
	
	TRACE_STATIC_METHOD();
	
	E(argc == 2, "Nginx.File#rename takes 2 mandatory arguments: from:String and to:String");
	
	
	// converting smth. to a string is a very common and rather simple operation,
	// so on failure it's very likely we have gone out of memory
	
	jss_from = JS_ValueToString(cx, JS_ARGV(cx, vp)[0]);
	if (jss_from == NULL)
	{
		JS_ReportOutOfMemory(cx);
		return JS_FALSE;
	}
	
	len = JS_EncodeStringToBuffer(jss_from, from, NGX_MAX_PATH - 1);
	if (len == (size_t) -1 || len > NGX_MAX_PATH - 1)
	{
		return JS_FALSE;
	}
	
	jss_to = JS_ValueToString(cx, JS_ARGV(cx, vp)[1]);
	if (jss_to == NULL)
	{
		JS_ReportOutOfMemory(cx);
		return JS_FALSE;
	}
	
	len = JS_EncodeStringToBuffer(jss_to, to, NGX_MAX_PATH - 1);
	if (len == (size_t) -1 || len > NGX_MAX_PATH - 1)
	{
		return JS_FALSE;
	}
	
	ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "ngx_rename_file(\"%s\", \"%s\")", from, to);
	JS_SET_RVAL(cx, vp, INT_TO_JSVAL(ngx_rename_file(from, to)));
	
	return JS_TRUE;
}

static JSBool
method_remove(JSContext *cx, uintN argc, jsval *vp)
{
	JSString        *jss_name;
	char            *name;
	
	TRACE_STATIC_METHOD();
	
	E(argc == 1, "Nginx.File#remove takes 1 mandatory argument: name:String");
	
	
	// converting smth. to a string is a very common and rather simple operation,
	// so on failure it's very likely we have gone out of memory
	
	jss_name = JS_ValueToString(cx, JS_ARGV(cx, vp)[0]);
	if (jss_name == NULL)
	{
		// forward exception if any
		return JS_FALSE;
	}
	
	name = JS_EncodeString(cx, jss_name);
	if (name == NULL)
	{
		return JS_FALSE;
	}
	
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "ngx_delete_file(\"%s\")", name);
	JS_SET_RVAL(cx, vp, INT_TO_JSVAL(ngx_delete_file(name)));
	
	JS_free(cx, name);
	
	return JS_TRUE;
}

static JSBool
method_exists(JSContext *cx, uintN argc, jsval *vp)
{
	JSString        *jss_name;
	char             name[NGX_MAX_PATH];
	size_t           len;
	ngx_file_info_t  fi;
	
	TRACE_STATIC_METHOD();
	
	E(argc == 1, "Nginx.File#exists takes 1 mandatory argument: name");
	
	jss_name = JS_ValueToString(cx, JS_ARGV(cx, vp)[0]);
	if (jss_name == NULL)
	{
		JS_ReportOutOfMemory(cx);
		return JS_FALSE;
	}
	
	len = JS_EncodeStringToBuffer(jss_name, name, NGX_MAX_PATH - 1);
	if (len == (size_t) -1 || len > NGX_MAX_PATH - 1)
	{
		return JS_FALSE;
	}
	
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, js_log(), 0, "ngx_file_info(\"%s\")", name);
	
	if (ngx_file_info((const char *) name, &fi) == NGX_FILE_ERROR)
	{
		JS_SET_RVAL(cx, vp, JSVAL_NULL);
		return JS_TRUE;
	}
	
	if (ngx_is_file(&fi))
	{
		JS_SET_RVAL(cx, vp, JSVAL_TRUE);
		return JS_TRUE;
	}
	
	JS_SET_RVAL(cx, vp, JSVAL_FALSE);
	return JS_TRUE;
}


static JSBool
method_getAccess(JSContext *cx, uintN argc, jsval *vp)
{
	JSString        *jss_name;
	char             name[NGX_MAX_PATH];
	size_t           len;
	ngx_file_info_t  fi;
	
	TRACE_STATIC_METHOD();
	
	E(argc == 1, "Nginx.File#getAccess takes 1 mandatory argument: name:String");
	
	jss_name = JS_ValueToString(cx, JS_ARGV(cx, vp)[0]);
	if (jss_name == NULL)
	{
		return JS_FALSE;
	}
	
	len = JS_EncodeStringToBuffer(jss_name, name, NGX_MAX_PATH - 1);
	if (len == (size_t) -1 || len > NGX_MAX_PATH - 1)
	{
		return JS_FALSE;
	}
	
	ngx_log_debug1(NGX_LOG_DEBUG_HTTP, js_log(), 0, "ngx_file_info(\"%s\")", name);
	if (ngx_file_info((const char *) name, &fi) == NGX_FILE_ERROR)
	{
		JS_SET_RVAL(cx, vp, JSVAL_NULL);
		return JS_TRUE;
	}
	
	JS_SET_RVAL(cx, vp, INT_TO_JSVAL(ngx_file_access(&fi)));
	return JS_TRUE;
}


static JSBool
method_setAccess(JSContext *cx, uintN argc, jsval *vp)
{
	ngx_uint_t       access;
	JSString        *jss_name;
	jsdouble         dp;
	char             name[NGX_MAX_PATH];
	size_t           len;
	
	TRACE_STATIC_METHOD();
	
	E(argc == 2, "Nginx.File#setAccess takes 2 mandatory arguments: name:String and access:Number");
	
	jss_name = JS_ValueToString(cx, JS_ARGV(cx, vp)[0]);
	if (jss_name == NULL)
	{
		return JS_FALSE;
	}
	
	len = JS_EncodeStringToBuffer(jss_name, name, NGX_MAX_PATH - 1);
	if (len == (size_t) -1 || len > NGX_MAX_PATH - 1)
	{
		return JS_FALSE;
	}
	
	if (!JS_ValueToNumber(cx, JS_ARGV(cx, vp)[1], &dp))
	{
		return JS_FALSE;
	}
	
	access = dp;
	
	ngx_log_debug2(NGX_LOG_DEBUG_HTTP, js_log(), 0, "ngx_change_file_access(\"%s\", %d)", name, access);
	JS_SET_RVAL(cx, vp, INT_TO_JSVAL(ngx_change_file_access((const char *) name, access)));
	
	return JS_TRUE;
}


static JSBool
method_write(JSContext *cx, uintN argc, jsval *vp)
{
	JSString        *jss_str;
	JSObject        *self;
	char            *str;
	ngx_fd_t         fd;
	size_t           len;
	void            *p;
	
	GET_PRIVATE(p);
	fd = PTR_TO_FD(p);
	TRACE_METHOD();
	
	E(argc == 1, "Nginx.File#write takes 1 mandatory argument: data:String");
	
	
	// converting smth. to a string is a very common and rather simple operation,
	// so on failure it's very likely we have gone out of memory
	
	jss_str = JS_ValueToString(cx, JS_ARGV(cx, vp)[0]);
	if (jss_str == NULL)
	{
		JS_ReportOutOfMemory(cx);
		return JS_FALSE;
	}
	
	str = JS_EncodeString(cx, jss_str);
	if (str == NULL)
	{
		return JS_FALSE;
	}
	
	len = ngx_strlen(str);
	
	ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "ngx_write_fd(fd=%d, len=%d)", fd, len);
	JS_SET_RVAL(cx, vp, ngx_write_fd(fd, str, len) == (ssize_t) len ? JSVAL_TRUE : JSVAL_FALSE);
	JS_free(cx, str);
	
	return JS_TRUE;
}

static JSBool
method_read(JSContext *cx, uintN argc, jsval *vp)
{
	ngx_fd_t         fd;
	JSString        *value;
	JSObject        *self;
	void            *p;
	size_t           len;
	char            *buf;
	
	GET_PRIVATE(p);
	fd = PTR_TO_FD(p);
	TRACE_METHOD();
	
	E(argc == 1 && JSVAL_IS_INT(JS_ARGV(cx, vp)[0]), "Nginx.File#read takes 1 mandatory argument: length:Number");
	
	len = JSVAL_TO_INT(JS_ARGV(cx, vp)[0]);
	if (len == 0)
	{
		return JS_TRUE;
	}
	
	buf = JS_malloc(cx, len);
	if (buf == NULL)
	{
		return JS_FALSE;
	}
	
	len = ngx_read_fd(fd, buf, len);
	value = JS_NewStringCopyN(cx, buf, len);
	JS_free(cx, buf);
	if (value == NULL)
	{
		return JS_FALSE;
	}
	
	JS_SET_RVAL(cx, vp, STRING_TO_JSVAL(value));
	return JS_TRUE;
}

static JSBool
method_seek(JSContext *cx, uintN argc, jsval *vp)
{
	ngx_fd_t         fd;
	void            *p;
	size_t           offset;
	JSObject        *self;
	
	GET_PRIVATE(p);
	fd = PTR_TO_FD(p);
	TRACE_METHOD();
	
	E(argc == 1 && JSVAL_IS_INT(JS_ARGV(cx, vp)[0]), "Nginx.File#seek takes 1 mandatory argument: offset:Number");
	
	offset = JSVAL_TO_INT(JS_ARGV(cx, vp)[0]);
	
	JS_SET_RVAL(cx, vp, INT_TO_JSVAL(lseek(fd, offset, SEEK_SET)));
	return JS_TRUE;
}

static JSBool
method_close(JSContext *cx, uintN argc, jsval *vp)
{
	ngx_fd_t         fd;
	void            *p;
	JSObject        *self;
	
	GET_PRIVATE(p);
	fd = PTR_TO_FD(p);
	TRACE_METHOD();
	
	if (ngx_close_file(fd) == NGX_FILE_ERROR)
	{
		ngx_log_error(NGX_LOG_ALERT, ngx_cycle->log, ngx_errno, ngx_close_file_n " Nginx.File (fd=%d, self=%p) failed", fd, self);
		JS_SET_RVAL(cx, vp, JSVAL_FALSE);
		return JS_TRUE;
	}
	
	JS_SetPrivate(cx, self, NULL);
	
	open_files--;
	
	JS_SET_RVAL(cx, vp, JSVAL_TRUE);
	return JS_TRUE;
}


static JSBool
getter_size(JSContext *cx, JSObject *self, jsid id, jsval *vp)
{
	ngx_fd_t          fd;
	void             *p;
	ngx_file_info_t   fi;
	off_t             size;
	
	GET_PRIVATE(p);
	fd = PTR_TO_FD(p);
	TRACE();
	
	if (ngx_fd_info(fd, &fi) != NGX_FILE_ERROR)
	{
		size = ngx_file_size(&fi);
		*vp = DOUBLE_TO_JSVAL(size);
	}
	
	return JS_TRUE;
}


static JSBool
constructor(JSContext *cx, uintN argc, jsval *vp)
{
	TRACE();
	JS_SetPrivate(cx, JS_THIS_OBJECT(cx, vp), NULL);
	JS_SET_RVAL(cx, vp, JSVAL_VOID);
	return JS_TRUE;
}

static void
finalizer(JSContext *cx, JSObject *self)
{
	ngx_fd_t         fd;
	void            *p;
	
	p = JS_GetInstancePrivate(cx, self, private_class, NULL);
	if (p == NULL)
	{
		ngx_log_debug0(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "finalizing File object with NULL private pointer");
		return;
	}
	
	fd = PTR_TO_FD(p);
	TRACE();
	
	if (ngx_close_file(fd) != NGX_FILE_ERROR)
	{
		open_files--;
	}
	else
	{
		ngx_log_error(NGX_LOG_ALERT, ngx_cycle->log, ngx_errno, ngx_close_file_n " Nginx.File (fd=%d, self=%p) failed", fd, self);
	}
	
	JS_SetPrivate(cx, self, NULL);
}

static JSBool
getter_readLeaks(JSContext *cx, JSObject *self, jsid id, jsval *vp)
{
	TRACE();
	
	*vp = INT_TO_JSVAL(read_leaks);
	
	return JS_TRUE;
}

static JSBool
getter_openFiles(JSContext *cx, JSObject *self, jsid id, jsval *vp)
{
	TRACE();
	
	*vp = INT_TO_JSVAL(open_files);
	
	return JS_TRUE;
}

static JSFunctionSpec funcs[] =
{
	JS_FS("write",           method_write,               1, 0, 0),
	JS_FS("read",            method_read,                1, 0, 0),
	JS_FS("seek",            method_seek,                1, 0, 0),
	JS_FS("close",           method_close,               0, 0, 0),
	JS_FS_END
};

static JSPropertySpec props[] =
{
	{"size",  0,  JSPROP_READONLY, getter_size, NULL},
	{0, 0, 0, NULL, NULL}
};


static JSFunctionSpec static_funcs[] =
{
	JS_FS("rename",           method_rename,               2, 0, 0),
	JS_FS("open",             method_open,                 1, 0, 0),
	JS_FS("remove",           method_remove,               1, 0, 0),
	JS_FS("exists",           method_exists,               1, 0, 0),
	JS_FS("getAccess",        method_getAccess,            1, 0, 0),
	JS_FS("setAccess",        method_setAccess,            2, 0, 0),
	JS_FS_END
};

static JSPropertySpec static_props[] =
{
	{"readLeaks",    0, JSPROP_READONLY, getter_readLeaks, NULL},
	{"openFiles",    0, JSPROP_READONLY, getter_openFiles, NULL},
	{0, 0, 0, NULL, NULL}
};

JSClass ngx_http_js__nginx_file__class =
{
	"File",
	JSCLASS_HAS_PRIVATE,
	JS_PropertyStub, JS_PropertyStub, JS_PropertyStub, JS_StrictPropertyStub,
	JS_EnumerateStub, JS_ResolveStub, JS_ConvertStub, finalizer,
	JSCLASS_NO_OPTIONAL_MEMBERS
};

JSBool
ngx_http_js__nginx_file__init(JSContext *cx, JSObject *global)
{
	static int  inited = 0;
	JSObject    *nginxobj, *fileobj;
	jsval        vp;
	
	if (inited)
	{
		JS_ReportError(cx, "Nginx.File is already inited");
		return JS_FALSE;
	}
	
	E(JS_GetProperty(cx, global, "Nginx", &vp), "global.Nginx is undefined");
	nginxobj = JSVAL_TO_OBJECT(vp);
	
	ngx_http_js__nginx_file__prototype = JS_InitClass(cx, nginxobj, NULL, &ngx_http_js__nginx_file__class,  constructor, 0,
		props, funcs, static_props, static_funcs);
	E(ngx_http_js__nginx_file__prototype, "Can`t JS_InitClass(Nginx.File)");
	
	E(JS_GetProperty(cx, nginxobj, "File", &vp), "global.Nginx.File is undefined");
	fileobj = JSVAL_TO_OBJECT(vp);
	
	
	
	vp = INT_TO_JSVAL(NGX_FILE_RDONLY);
	JS_SetProperty(cx, fileobj, "RDONLY", &vp);
	
	vp = INT_TO_JSVAL(NGX_INVALID_FILE);
	JS_SetProperty(cx, fileobj, "INVALID", &vp);
	
	vp = INT_TO_JSVAL(NGX_FILE_ERROR);
	JS_SetProperty(cx, fileobj, "ERROR", &vp);
	
#ifdef NGX_HAVE_CASELESS_FILESYSTEM
	vp = JSVAL_TRUE;
#else
	vp = JSVAL_FALSE;
#endif
	JS_SetProperty(cx, fileobj, "HAVE_CASELESS_FILESYSTEM", &vp);
	
	vp = INT_TO_JSVAL(NGX_FILE_RDONLY);
	JS_SetProperty(cx, fileobj, "RDONLY", &vp);
	
	vp = INT_TO_JSVAL(NGX_FILE_WRONLY);
	JS_SetProperty(cx, fileobj, "WRONLY", &vp);
	
	vp = INT_TO_JSVAL(NGX_FILE_RDWR);
	JS_SetProperty(cx, fileobj, "RDWR", &vp);
	
	vp = INT_TO_JSVAL(NGX_FILE_CREATE_OR_OPEN);
	JS_SetProperty(cx, fileobj, "CREATE_OR_OPEN", &vp);
	
	vp = INT_TO_JSVAL(NGX_FILE_OPEN);
	JS_SetProperty(cx, fileobj, "OPEN", &vp);
	
	vp = INT_TO_JSVAL(NGX_FILE_TRUNCATE);
	JS_SetProperty(cx, fileobj, "TRUNCATE", &vp);
	
	vp = INT_TO_JSVAL(NGX_FILE_APPEND);
	JS_SetProperty(cx, fileobj, "APPEND", &vp);
	
	vp = INT_TO_JSVAL(NGX_FILE_NONBLOCK);
	JS_SetProperty(cx, fileobj, "NONBLOCK", &vp);
	
	vp = INT_TO_JSVAL(NGX_FILE_DEFAULT_ACCESS);
	JS_SetProperty(cx, fileobj, "DEFAULT_ACCESS", &vp);
	
	vp = INT_TO_JSVAL(NGX_FILE_OWNER_ACCESS);
	JS_SetProperty(cx, fileobj, "OWNER_ACCESS", &vp);
	
	inited = 1;
	
	return JS_TRUE;
}
